...
environ['JAVA_HOME'] = '/usr/lib/jvm/jdk-11-oracle-x64'
environ['PATH'] += ':/usr/lib/jvm/jdk-11-oracle-x64/bin'

dir_path = os.path.dirname(os.path.realpath(__file__))
print(dir_path)
home_path = str(Path.home())
EXTRACTOR_JAR = '/home/t-anthonyp/repos/features-javac/extractor/target/features-javac-extractor-1.0.0-SNAPSHOT-jar-with-dependencies.jar'
...
DOT_JAR = '/home/t-anthonyp/repos/features-javac/dot/target/features-javac-dot-1.0.0-SNAPSHOT-jar-with-dependencies.jar'
...
def build_java_graphs():
    ...
    for type in types:
        ...
        if not os.path.exists(java_func_dir):
            # sudo_makedirs(java_func_dir)
            os.makedirs(java_func_dir)
        results = parallel_process(samples, build_single_graph, args=(java_func_dir, True, ))
        succeed = 0
...
def build_single_graph(sample, java_func_dir, override=False):
    try:
...
def generate_proto_file(wrapped_file_path):
    command = '/usr/lib/jvm/jdk-11-oracle-x64/bin/javac -cp %s -Xplugin:FeaturePlugin %s' % (EXTRACTOR_JAR, wrapped_file_path)
    # command = 'javac -cp %s -Xplugin:FeaturePlugin %s' % (EXTRACTOR_JAR, wrapped_file_path)
...
def generate_dot_json_file(wrapped_file_path):
    command = '/usr/lib/jvm/jdk-11-oracle-x64/bin/java -jar %s -i %s -o %s -j %s' % (DOT_JAR,
...
def single_post_process(key, chunk):
    ...
    with gzip.GzipFile(file_graph_name, 'wb') as gnn_file:
        for file in tqdm(chunk, desc="Single Post Process"):
...
def single_post_base_process(files, index):
    ...
    with gzip.GzipFile(os.path.join(BASE_GRAPH_FILE, 'base_graph_gnn_' + str(index) + '.jsonl.gz'), 'wb') as gnn_file:
        for file in tqdm(files, desc="Single Post Base Process"):
            class_name = file.split('_')[1]
            try:
